...
const _FS = `
  ...
  constructor(lerp: { (t: any, a: any, b: any): any; (t: any, a: any, b: any): any; (t: any, a: any, b: any): any; }) {
    this.points_ = [];
    this._lerp = lerp;
  }

  AddPoint(t: number, d: THREE.Color | number) {
...
export class ParticleEmitter {
  ...
  constructor() {
    this.alphaSpline_ = new LinearSpline((t: number, a: number, b: number) => {
      return a + t * (b - a);
    });

    this.colourSpline_ = new LinearSpline((t: any, a: { clone: () => any; }, b: any) => {
      const c = a.clone();
      return c.lerp(b, t);
    });

    this.sizeSpline_ = new LinearSpline((t: number, a: number, b: number) => {
      return a + t * (b - a);
    });
...
export class ParticleSystem {
  AddParticles(Position: any, arg1: number) {
  ...
  constructor(params: { texture: any; camera: any; parent: { add: (arg0: any) => void; }; }) {
    const uniforms = {
        diffuseTexture: {
  ...
  UpdateParticles_(timeElapsed: any) {
  ...
  UpdateEmitters_(timeElapsed: any) {
