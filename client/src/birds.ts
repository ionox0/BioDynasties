// Todo: npm types for dat.gui and three-addons are not available
//@ts-nocheck
...
import { GPUComputationRenderer } from "../node_modules/three/examples/jsm/misc/GPUComputationRenderer";
...
const gltfs = ["resources/nature/Parrot.glb"]; //, "resources/nature/Flamingo.glb"];
...
let textureAnimation: any;
let durationAnimation: any;
let birdMesh: any;
let materialShader: any;
let indicesPerBird: any;
...
export class Birds {
  constructor(mainC) {
    ...
    renderer = mainC.threejs_;
    camera = mainC.camera_;
    scene = mainC.scene_;
  }
  ...
  loadBirds() {
    ...
    new GLTFLoader().load(gltfs[selectModel], function (gltf) {
      ...
      const tHeight = that.nextPowerOf2(durationAnimation);
      const tWidth = that.nextPowerOf2(birdGeo.getAttribute("position").count);
