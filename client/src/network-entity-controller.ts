...
export class NetworkEntityController extends Component {
  ...
  InitComponent() {
    this._RegisterHandler('network.update', (m: any) => { this.OnNetworkUpdate_(m); });
  }

  SetTransform_(transform: any[]) {
  ...
  OnNetworkUpdate_(msg: { transform: any; stats: any; events: string | any[]; }) {
    if ('transform' in msg) {
      this.lastUpdate_ = 0.0;
