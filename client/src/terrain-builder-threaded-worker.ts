...
export class TerrainBuilderThreadedWorker {
  ...
  Init(params: { offset: any[]; noiseParams: any; biomesParams: any; colourNoiseParams: any; }) {
    this._params = params;
  ...
  Rebuild() {
    ...
    for (let i = 0, n = indices.length; i < n; i+=3) {
      ...
      const splatStrengths: {[k: string]: any} = {};
      ...
      let typeValues: any = Object.values(splatStrengths);
      typeValues.sort((a: { strength: number; }, b: { strength: number; }) => {
