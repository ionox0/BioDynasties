...
export const spatial_hash_grid = (() => {
  ...
  class SpatialHashGrid {
    ...
    constructor(bounds: any, dimensions: [any, any]) {
    ...
    UpdateClient(client: { position: [any, any]; dimensions: [any, any]; _cells: { min: number[]; max: number[]; }; }) {
    ...
    FindNear(position: [any, any], bounds: [any, any]) {
      const [x, y] = position;
      const [w, h] = bounds;
    ...
    _Insert(client: { position: any; dimensions: any; _cells: any; _queryId?: number; }) {
      const [x, y] = client.position;
      const [w, h] = client.dimensions;
